...
SELECT m.pmovim, m.nro_patente, m.COD_ENDOSO, m.endoso, m.poliza, v.cPoliza, m.fecha_inicio_vig, v.dIniVigencia, datediff(m.fecha_inicio_vig, v.dIniVigencia) nDiff, v.fTpDispositivo, v.cIdDispositivo, v.bVigente, v.tBaja
...
and ( m.COD_ENDOSO is NULL OR m.ENDOSO='00000')
order by nro_patente, fecha_inicio_vig;

-- Muestra los movimientos que no tienen vehiculo con la misma poliza
SELECT m.pmovim, m.nro_patente, m.COD_ENDOSO, m.endoso, m.poliza, m.fecha_inicio_vig, m.COD_TIPO_ESTADO, m.MAIL
...
-- and datediff(m.fecha_inicio_vig, v.dIniVigencia) <> 0
-- and IFNULL(m.COD_ENDOSO,'0000') in ( '0000','2001','PR00' )
order by nro_patente, fecha_inicio_vig;
