...
-- 				AND		v.bVigente = '1';
    ...
    UPDATE	wMemoryCierreTransf
    SET		nDiasNoSincro = DATEDIFF( LEAST( DATE(fnNowTest()), dProximoCierreIni )
                              , GREATEST( IFNULL(DATE( tUltTransferencia), '0000-00-00')
                                        , IFNULL(DATE( tUltViaje        ), '0000-00-00')
                                        , IFNULL(DATE( tUltControl      ), '0000-00-00')) )
		,	nDiasAlCierre = DATEDIFF(dProximoCierreIni,DATE(fnNowTest())) + ( CASE WHEN TIMESTAMPDIFF(MONTH,dIniVigencia, dProximoCierreIni) < 1 THEN DAY(LAST_DAY(fnNowTest())) ELSE 0 END );
END //
