...
select t.id                     AS trip_id          , t.client_id              AS client_id
     ...
     , ( case when isnull(so.display_name) then so.name else so.display_name end ) AS calle
     , ( case when isnull(st.display_name) then st.name else st.display_name end ) AS calle_inicio
